...
print_single_rel(PlannerInfo *root, RelOptInfo *rel, double orows) {
	...
	fprintf(f_rec, "Original extimate: %lf\n", orows);
	fprintf(f_rec, "Ori sel extimate: %.15f\n", rel->rows / orows);
    fprintf(f_rec, "\n\n");
    fclose(f_rec);
}
...
print_join_rel(PlannerInfo *root, RelOptInfo *rel1, RelOptInfo *rel2, double nrows){
	...
	fprintf(f_rec, "Original extimate: %lf\n", nrows);
	fprintf(f_rec, "Ori sel extimate: %lf\n", nrows / (rel1->rows * rel2->rows));
...
//        print_query_no(__FUNCTION__);
		...
		// if query no in file, then do following
	...
	if(print_single_tbl_queries){
        print_single_rel(root, rel, orows);
	}
	query_no++;
						   ...
						   List *restrictlist_in)
{
    ...
    if (ml_joinest_enabled) {
		...
		if (mypointers[query_no + join_est_no] > 0){
			...
			// if (true) {
			// 	FILE *ffp = fopen("going_to_change_join.txt", "a+");
			// 	fprintf(ffp, "%d  %d  %.5f:%.5f\n", query_no, join_est_no, nrows, join_est);
			// 	fclose(ffp);
			// }
